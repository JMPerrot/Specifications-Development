...
# save results
for (parm in ParmsOfInterest){
  FileName <- file.path(PathResults,paste(parm,'_REFERENCE#1T.RData',sep = ''))
...
for (parm in ParmsOfInterest){
  ...
  fileName <- file.path(PathResults,'FIGURES', paste(parm,'_REFERENCE#1T.png',sep = ''))
...
fileName <- file.path(PathResults,'FIGURES', 'All_REFERENCE#1T.png')
