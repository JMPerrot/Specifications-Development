...
scatter_inversion <- function(target, estimate, Colors, Labs, fileName,
  ...
  if (PlotStats == TRUE){
    ...
    for (conf in 1){#BP_df[[BPvar]]$SpectralConf)){
      case <- case+1
      my.label.expr = paste0("italic(R)^ 2 == ", as.numeric(format(round(Stats$R2, 2), nsmall = 2)),sep='')
      ...
      my.label.expr = paste0("italic(NRMSE) == ", as.numeric(format(round(Stats$NRMSE, 2), nsmall = 2)))
      Splot <- Splot + annotate(geom = "text",
                                x = MinMaxAxis[1] +0.02*(MinMaxAxis[2]-MinMaxAxis[1]),
