...
################################################################################
PathData <- '../../../01_DATA'
PathResults <- '../../../03_RESULTS/R_only_N_Prior/02_SpecSampling'
...
SpectralSampling_subDir <- file.path(PathResults,'_individualSteps')
...
for (parm in names(Estimate_SpectralSampling)){
  Labs <- c(paste('Measured',parm,UnitsParms[[parm]]), 
            paste('Estimated_R_Only',parm,UnitsParms[[parm]]))
