...
SHIFT_Dir_Fig_EWT <- file.path(SHIFT_Dir,paste('FIGURES/EWT/EWT_',pas,'nm', sep =""))
...
SHIFT_Dir_Data<-paste(SHIFT_Dir,paste("/pas=",pas,"nm_mat", sep=""), sep = "")
dir.create(paste(SHIFT_Dir,paste("/pas=",pas,"nm_mat", sep=""), sep = ""),showWarnings = F,recursive = T)

for (shift in shift_list) {
  if (!file.exists(paste(SHIFT_Dir,paste("/pas=",pas,"nm_mat/SHIFT=",shift,"nm",".RData", sep=""), sep = ""))){
    ############################################################################
    # definition of the sampling step
    ############################################################################
    wl<-seq(1300+shift,2450, by = pas)
    ...
    ############################################################################
    parms_est_df=data.frame("EWT" = NA, "LMA" = NA, "ANT" = NA,
                            "EWT" = NA, "LMA" = NA, "N" = NA)
    Parms2Estimate  = c('EWT','LMA','ANT','EWT','LMA') 
    #we are estimating all parameters but spectral band is not optimized for LMA and EWT
    
    InitValues <- data.frame(EWT=45, LMA=8, ANT=0.1, BROWN=0, EWT=0.01, LMA=0.01, N=1.5)
